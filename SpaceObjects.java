...
class Space {
	...
	public void queryCollisions() {
		...
		for (Asteroid asteroid : asteroids) {
			for (Wall wall : walls) {
				if (Space.Physics.collide(asteroid.getShape(), wall.getShape())) {
					Space.Physics.colliding(asteroid, wall);
				}
			}
		}
		...
		for (Ship ship : ships) {
			for (Wall wall : walls) {
				if (Space.Physics.collide(ship.getShape(), wall.getShape())) {
					Space.Physics.colliding(ship, wall);
				}
			}
		}
		...
		for (Ship.Bullet bullet : bullets) {
			for (Wall wall : walls) {
				if (Space.Physics.collide(bullet.getShape(), wall.getShape())) {
					Space.Physics.colliding(bullet, wall);
				}
			}
		}
	}
